...
$stmt = $conn->prepare("SELECT conteudo_novo FROM termos_historico WHERE termos_id = ? AND versao = ? ORDER BY data_alteracao DESC LIMIT 1");
...
$stmt->execute();
$termos = $stmt->get_result()->fetch_assoc();
...
// Gera nova versão
$nova_versao = gerarNovaVersao($termos['versao']);
...
$stmt_hist->bind_param("issssi", $id, $nova_versao, $termos['conteudo'], $historico['conteudo_novo'], $alteracoes, $_SESSION['admin_id']);
$stmt_hist->execute();
