<?php
$page_title = 'Editar Termos de Uso';
...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if(empty($erros)) {
        ...
        $stmt_hist = $conn->prepare("
            INSERT INTO termos_historico (termos_id, versao, conteudo_antigo, conteudo_novo, alteracoes, alterado_por, data_alteracao)
            VALUES (?, ?, ?, ?, ?, ?, NOW())
        ");
        $alteracoes = "Editado por administrador";
        $stmt_hist->bind_param("issssi", $id, $termos['versao'], $termos['conteudo'], $conteudo, $alteracoes, $_SESSION['admin_id']);
        $stmt_hist->execute();
        
        $data_publicacao = $termos['data_publicacao'];
        if($status == 'publicado' && $termos['status'] != 'publicado') {
            $data_publicacao = date('Y-m-d H:i:s');
        }
        
        $stmt = $conn->prepare("
            UPDATE termos_uso SET 
        ...
        if($stmt->execute()) {
            ...
            if($status == 'publicado') {
                $conn->query("UPDATE termos_uso SET status = 'arquivado' WHERE id != $id AND status = 'publicado'");
            }
...
<div class="main-content" id="mainContent">
    <div class="top-bar">
        <h1 class="page-title">
            ...
            Editando: <?php echo $termos['titulo']; ?>
        </h1>
    </div>
                ...
                <form method="POST">
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="text" name="titulo" class="form-control" required value="<?php echo $termos['titulo']; ?>">
                        </div>
                        
                        <div class="form-group">
                            <label><i class="fas fa-code-branch"></i> Versão</label>
                            <input type="text" name="versao" class="form-control" value="<?php echo $termos['versao']; ?>">
                        </div>
                    </div>

                    <div class="form-group">
                        <label><i class="fas fa-quote-right"></i> Subtítulo</label>
                        <input type="text" name="subtitulo" class="form-control" value="<?php echo $termos['subtitulo']; ?>">
                    </div>

                    <div class="form-group">
                        <label><i class="fas fa-code"></i> Status</label>
                        <select name="status" class="form-control">
                            <option value="rascunho" <?php echo $termos['status'] == 'rascunho' ? 'selected' : ''; ?>>Rascunho</option>
                            <option value="publicado" <?php echo $termos['status'] == 'publicado' ? 'selected' : ''; ?>>Publicado</option>
                            <option value="arquivado" <?php echo $termos['status'] == 'arquivado' ? 'selected' : ''; ?>>Arquivado</option>
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="conteudo" class="form-control" required id="conteudo"><?php echo htmlspecialchars($termos['conteudo']); ?></textarea>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="meta_title" class="form-control" value="<?php echo $termos['meta_title']; ?>">
                    </div>

                    <div class="form-group">
                        <label><i class="fas fa-paragraph"></i> Meta Description</label>
                        <textarea name="meta_description" class="form-control" rows="3"><?php echo $termos['meta_description']; ?></textarea>
                    </div>

                    <div class="form-group">
                        <label><i class="fas fa-key"></i> Meta Keywords</label>
                        <input type="text" name="meta_keywords" class="form-control" value="<?php echo $termos['meta_keywords']; ?>">
                    </div>
