...
if(isset($_GET['acao'])) {
    ...
    if($_GET['acao'] == 'excluir') {
        $conn->query("DELETE FROM politica_secoes WHERE id = $id");
    ...
    if($_GET['acao'] == 'up') {
        ...
        $secao = $conn->query("SELECT ordem FROM politica_secoes WHERE id = $id")->fetch_assoc();
        $nova_ordem = $secao['ordem'] - 1;
        $conn->query("UPDATE politica_secoes SET ordem = ordem + 1 WHERE ordem = $nova_ordem AND politica_id = {$politica_ativa['id']}");
        $conn->query("UPDATE politica_secoes SET ordem = $nova_ordem WHERE id = $id");
        header('Location: secoes.php');
        exit;
    }
...
$secoes = $conn->query("
    SELECT * FROM politica_secoes 
    WHERE politica_id = {$politica_ativa['id']} 
    ORDER BY ordem
");
    ...
    <div class="content-area">
        ...
        <div class="politica-info">
            ...
            <div class="info-content">
                <h3>Política: <?php echo $politica_ativa['titulo']; ?></h3>
                <p>Adicione seções para organizar melhor o conteúdo da sua política de privacidade</p>
            </div>
            <span class="versao-badge">v<?php echo $politica_ativa['versao']; ?></span>
        </div>
    ...
    <div class="modal-content">
        ...
        <form method="POST" action="salvar_secao.php" id="secaoForm">
            <div class="modal-body">
                ...
                <input type="hidden" name="politica_id" value="<?php echo $politica_ativa['id']; ?>">
