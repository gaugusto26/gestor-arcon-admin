<?php
$page_title = 'Nova Política de Privacidade';
...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if(empty($erros)) {
        ...
        if($stmt->execute()) {
            $politica_id = $conn->insert_id;
            
            // Se for publicado, despublica outras versões
            if($status == 'publicado') {
                $conn->query("UPDATE politica_privacidade SET status = 'arquivado' WHERE id != $politica_id AND status = 'publicado'");
            }
                ...
                <form method="POST">
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="text" name="titulo" class="form-control" required value="<?php echo $_POST['titulo'] ?? 'Política de Privacidade'; ?>">
                        </div>
...
<p>Em caso de dúvidas sobre esta política, entre em contato através do e-mail: sistemasntw@gmail.com</p>'; ?></textarea>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="meta_title" class="form-control" value="<?php echo $_POST['meta_title'] ?? 'Política de Privacidade | NTW - New Software'; ?>">
                    </div>

                    <div class="form-group">
                        <label><i class="fas fa-paragraph"></i> Meta Description</label>
                        <textarea name="meta_description" class="form-control" rows="3"><?php echo $_POST['meta_description'] ?? 'Conheça nossa política de privacidade e saiba como protegemos seus dados na NTW - New Software.'; ?></textarea>
                    </div>
