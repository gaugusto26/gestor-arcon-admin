...
$politica_id = (int)$_POST['politica_id'];
...
if($id > 0) {
    // Editar
    $stmt = $conn->prepare("UPDATE politica_secoes SET titulo = ?, icone = ?, conteudo = ? WHERE id = ?");
    $stmt->bind_param("sssi", $titulo, $icone, $conteudo, $id);
} else {
    // Criar - pega a última ordem
    $result = $conn->query("SELECT MAX(ordem) as max_ordem FROM politica_secoes WHERE politica_id = $politica_id");
    $row = $result->fetch_assoc();
    $nova_ordem = ($row['max_ordem'] ?? -1) + 1;
    
    $stmt = $conn->prepare("INSERT INTO politica_secoes (politica_id, titulo, icone, conteudo, ordem) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("isssi", $politica_id, $titulo, $icone, $conteudo, $nova_ordem);
}
